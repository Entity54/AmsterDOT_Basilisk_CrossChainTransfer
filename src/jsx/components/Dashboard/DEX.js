...
const DEX = ({
  ...
  const [destinationChainSelected, setDestinationChainSelected] = useState(
    'Basilisk',
  )
  ...
  const getBalancesAndAccountFormats = async (
    account,
    token = null,
    ...
    console.log(
      `Getting account formats for account: ${account} and balance for token: ${token} metamaskAccount: ${metamaskAccount} for network:${network} (if null we get balances for all supported networks)`,
    )
  ...
  const getROCOCO_BalancesAndAccountFormats = async (
    account,
    token = null,
    metamaskAccount = null,
    network = null,
    ...
    console.log(
      `Getting ROCOCO account formats for account: ${account} and balance for token: ${token} metamaskAccount: ${metamaskAccount} for network:${network} (if null we get balances for all supported networks)`,
    )
    const response = await getRococo_AvailableBalance(
      account,
      token,
      metamaskAccount,
      network,
  ...
  const getAllBalancesAndAccountFormats = async (account, metamaskAccount) => {
    const kusamaResponse = await getBalancesAndAccountFormats(
      account,
      'KSM',
      metamaskAccount,
    )
    if (kusamaResponse) {
      ...
      const { balances: balancesKAR } = await getBalancesAndAccountFormats(
        account,
        'KAR',
        metamaskAccount,
      )
      const { balances: balancesMOVR } = await getBalancesAndAccountFormats(
        account,
        'MOVR',
        metamaskAccount,
      )
      const { balances: balancesKINT } = await getBalancesAndAccountFormats(
        account,
        'KINT',
        metamaskAccount,
      )
      const { balances: balancesPHA } = await getBalancesAndAccountFormats(
        account,
        'PHA',
        metamaskAccount,
      )
      const { balances: balancesAUSD } = await getBalancesAndAccountFormats(
        account,
        'AUSD',
        metamaskAccount,
      )
      const { balances: balancesKBTC } = await getBalancesAndAccountFormats(
        account,
        'KBTC',
        metamaskAccount,
      )
      ...
      console.log(`Some API Sockets are not ready`)
      return
    }
  }

  const getROCOCO_AllBalancesAndAccountFormats = async (
    account,
    metamaskAccount,
  ) => {
    const basiliskResponse = await getROCOCO_BalancesAndAccountFormats(
      account,
      'BSX',
      metamaskAccount,
    )
    if (basiliskResponse) {
      ...
      const {
        balances: balancesPHA,
      } = await getROCOCO_BalancesAndAccountFormats(
        account,
        'PHA',
        metamaskAccount,
      )
      ...
      console.log(
        `getROCOCO_AllBalancesAndAccountFormats Some API Sockets are not ready`,
      )
      return
    }
  }
  ...
  useEffect(() => {
    ...
    const updateBalances = async () => {
      if (polakdotAccountSigner.address !== null && setupSpecs) {
        console.log(
          `New Polkadot account has been chosen polakdotAccountSigner.address:${polakdotAccountSigner.address} or new MeataMask address:${setupSpecs.walletAddress}`,
        )

        if (ecosystem === 'Kusama') {
          const balanceObj = await getAllBalancesAndAccountFormats(
            polakdotAccountSigner.address,
            setupSpecs.walletAddress,
          )
          if (
            tokenSelected !== 'Token' &&
            originChainSelected !== 'Origin Chain' &&
            tokenSelected !== 'LKSM=>KSM'
          ) {
            const availableToTransfer =
              balanceObj[`${tokenSelected}`][`${originChainSelected}`]
            setAvailableAmountToTransfer(availableToTransfer)
          }
        } else if (ecosystem === 'Rococo') {
          const balanceObj = await getROCOCO_AllBalancesAndAccountFormats(
            polakdotAccountSigner.address,
            setupSpecs.walletAddress,
          )
          if (
  ...
  const selectedOriginChainfunction = async (chainString) => {
    console.log(
      `DEX SelectedOriginChain is: ${chainString}  tokenSelected:${tokenSelected}`,
    )
    ...
    if (chainString !== 'Origin Chain' && tokenSelected !== 'Token') {
      if (ecosystem === 'Kusama') {
        ...
        const balanceObj = await getAllBalancesAndAccountFormats(
          polakdotAccountSigner.address,
          accountList[0],
        ) //accountList[0] is setupSpecs.walletAddress
        const availableToTransfer =
          balanceObj[`${tokenSelected}`][`${chainString}`]
        setAvailableAmountToTransfer(availableToTransfer)
      } else if (ecosystem === 'Rococo') {
        const balanceObj = await getROCOCO_AllBalancesAndAccountFormats(
          polakdotAccountSigner.address,
        ...
        console.log(
          `ROCOCO ====> balanceObj: `,
          balanceObj,
          `  <============ tokenSelected: ${tokenSelected} originChainSelected: ${chainString}`,
        )
            ...
            <Card>
              <Card.Body>
                ...
                <QuickTrade
                  ...
                  destinationChainSelected={destinationChainSelected}
                  originChainSelected={originChainSelected}
                  tokenSelected={tokenSelected}
                  relaySpecs={relaySpecs}
                  karuraAlphaSpecs={karuraAlphaSpecs}
                  blockHeader={blockHeader}
                  accountList={accountList}
                  polakdotAccountSigner={polakdotAccountSigner}
                  accountFormats={accountFormats}
