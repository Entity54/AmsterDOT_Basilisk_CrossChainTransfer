...
const QuickTrade = ({
  ...
  accountFormats,
  selectedAction,
  getAllBalancesAndAccountFormats,
  getROCOCO_AllBalancesAndAccountFormats,
}) => {
  ...
  const relayTokenPrecompileAddress =
    '0xFfFFfFff1FcaCBd218EDc0EbA20Fc2308C778080' //xcUNIT or xcKSM
  ...
  useEffect(() => {
    if (accountFormats) {
      if (originChainSelected === 'Kusama')
        setSuggestedOriginAccount(accountFormats.kusama_Address)
      else if (originChainSelected === 'Karura')
        setSuggestedOriginAccount(accountFormats.karura_Address)
      else if (originChainSelected === 'Kintsugi')
        setSuggestedOriginAccount(accountFormats.kintsugi_Address)
      else if (originChainSelected === 'Phala')
        setSuggestedOriginAccount(accountFormats.khala_Address)
      else if (originChainSelected === 'Basilisk')
        setSuggestedOriginAccount(accountFormats.basilisk_Address)
  ...
  useEffect(() => {
    ...
    if (originChainSelected === 'Moonriver')
      account_formats = getAccountFormatsforAccountI32(
        polakdotAccountSigner.address,
      )

    if (destinationChainSelected === 'Kusama')
      setSendToAddress(account_formats.kusama_Address)
    else if (destinationChainSelected === 'Karura')
      setSendToAddress(account_formats.karura_Address)
    else if (destinationChainSelected === 'Kintsugi')
      setSendToAddress(account_formats.kintsugi_Address)
    else if (destinationChainSelected === 'Phala')
      setSendToAddress(account_formats.khala_Address)
    else if (destinationChainSelected === 'Moonriver')
      setSendToAddress(accountList[0])
    else if (destinationChainSelected === 'Basilisk')
      setSendToAddress(account_formats.basilisk_Address)
  ...
  useEffect(() => {
    const getLKSbalance = async () => {
      if (selectedAction === 'unstakeKSMfromLKSMatKarura') {
        const availableBalanceLKSM = await getBalanceinKarura(
          polakdotAccountSigner.address,
          'LKSM',
        )
  ...
  const transferBalance = async (token, originChain, targetChain) => {
    ...
    console.log(
      `XCMTransfer Begins transferBalance baseCurrency: `,
      baseCurrency,
      ` sendToAddress: `,
      sendToAddress,
      `  inputTranferAmount: `,
      inputTranferAmount,
      ` orginChain: ${orginChain} targetChainDestination:${targetChainDestination}`,
    )
    ...
    if (
      ...
      if (baseCurrency === 'KSM') {
        ...
        if (orginChain === 'Kusama' && targetChainDestination === 'Karura') {
          console.log(
            `We are sending KSM from Kusama to Karura inputTranferAmount:${amount} sendToAddress:${sendToAddress}`,
          )
          setTransfer_IsSubmiting(true)
          setTransactionMessage(
            `Transfer KSM from Kusama to Karura, submitted.`,
          )
      ...
      //#region baseCurrency AUSD
        ...
        if (orginChain === 'Karura' && targetChainDestination === 'Phala') {
          ...
          setTransfer_IsSubmiting(true)
          setTransactionMessage(
            `Transfer AUSD from Karura to Phala, submitted.`,
          )

          transfer_Asset_FromParachainToParachain(
            'KUSD',
            parachainCodes.Karura,
            parachainCodes.Phala,
            sendToAddress,
            amount,
          )
            .then((resolveMsg) => {
      ...
      //#region baseCurrency BSX
        ...
        if (orginChain === 'Basilisk' && targetChainDestination === 'Phala') {
          console.log(
            `We are sending BSX from Basilisk to Phala inputTranferAmount:${amount} sendToAddress:${sendToAddress}`,
          )
          setTransfer_IsSubmiting(true)
          setTransactionMessage(
            `Transfer BSX from Basilisk to Phala, submitted.`,
          )

          rococo_transfer_Asset_FromParachainToParachain(
            'BSX',
            parachainCodes.Basilisk_Rococo,
            parachainCodes.Phala,
            sendToAddress,
            amount,
          )
          ...
          console.log(
            `We are sending BSX from Basilisk to Karura inputTranferAmount:${amount} sendToAddress:${sendToAddress}`,
          )
          setTransfer_IsSubmiting(true)
          setTransactionMessage(
            `Transfer BSX from Basilisk to Karura, submitted.`,
          )
          ...
          orginChain === 'Phala' &&
          targetChainDestination === 'Basilisk'
        ) {
          console.log(
            `We are sending BSX from Phala to Basilisk inputTranferAmount:${amount} sendToAddress:${sendToAddress}`,
          )
          setTransfer_IsSubmiting(true)
          setTransactionMessage(
            `Transfer BSX from Phala to Basilisk, submitted.`,
          )

          transferFromPhalaToParachain(
            'BSX',
            parachainCodes.Basilisk,
            parachainCodes.Basilisk,
            sendToAddress,
            amount,
          )
          ...
          console.log(
            `We are sending BSX from Karura to Basilisk inputTranferAmount:${amount} sendToAddress:${sendToAddress}`,
          )
          setTransfer_IsSubmiting(true)
          setTransactionMessage(
            `Transfer BSX from Karura to Basilisk, submitted.`,
          )

          // transfer_Asset_FromParachainToParachain("BSX", parachainCodes.Karura_Rococo, parachainCodes.Basilisk, sendToAddress, amount)
          rococo_transfer_Asset_FromParachainToParachain(
            'BSX',
            parachainCodes.Karura_Rococo,
            parachainCodes.Basilisk,
            sendToAddress,
            amount,
          )
              ...
              <div className="row mt-4" style={{ backgroundColor: '' }}>
                ...
                >
                  <button
                    ...
                    disabled={transfer_IsSubmiting || loading}
                    onClick={() =>
                      transferBalance(
                        tokenSelected,
                        originChainSelected,
