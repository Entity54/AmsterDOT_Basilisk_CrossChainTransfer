...
import {
  ...
  stringToHex,
  hexToU8a,
} from '@polkadot/util' // Some helper functions used here
...
const setApi = (apiName, api) => {
  ...
  else if (apiName === 'Karura') {
    KaruraApi = api
    apiObj['2000'] = api
  } else if (apiName === 'Moonriver') {
    MoonriverApi = api
    apiObj['2023'] = api
  } else if (apiName === 'Kintsugi') {
    KintsugiApi = api
    apiObj['2092'] = api
  } else if (apiName === 'Phala') {
    PhalaApi = api
    apiObj['2004'] = api
  } else if (apiName === 'Basilisk') {
    BasiliskApi = api
    apiObj['2090'] = api
  } else if (apiName === 'Shiden') {
    ShidenApi = api
    apiObj['2007'] = api
  } else if (apiName === 'Rococo_Phala') {
    Rococo_RhalaApi = api
    apiObj['r2004'] = api
  } else if (apiName === 'Rococo_Basilisk') {
    Rococo_BasiliskApi = api
    apiObj['r2090'] = api
  } else if (apiName === 'Rococo_Karura') {
    Rococo_KaruraApi = api
    apiObj['r2000'] = api
  }
}
...
let polkadotInjector = null,
  polkadotInjectorAddress = null
const setPolkadotInjector = (injector, injectorAddress) => {
  polkadotInjector = injector
  polkadotInjectorAddress = injectorAddress
  console.log(
    `Setup New Polkadot Signer/Injector polkadotInjectorAddress: ${polkadotInjectorAddress} polkadotInjector: `,
    polkadotInjector,
  )
}
...
const checkAllowanceOfTokentoAddress = async (
  wallet,
  tokenAddress,
  client,
  spender,
...
const approve = async (tokenAddress, spender, amountin_unitsEth = '0.001') => {
  ...
  const amountWEI = ethers.BigNumber.from(
    ethers.utils.parseUnits(amountin_unitsEth, 12),
  )
  console.log(
    `Client wants to transfer ${amountWEI} Tokens WEI. NEED TO APPROVE TRANSFER OF THIS TOKEN FROM CLIENT ACCOUNT TO SPENDER`,
  )
...
const simpleERC20Transfer = async (
  wallet,
  tokenAddress,
  receipient,
  amountin_unitsEth = '0.001',
) => {
  const erc20 = new ethers.Contract(tokenAddress, IERC20_raw.abi, wallet)
...
const transfer_multiasset = async (
  assetStr,
  assetParachainId,
  destParachainId,
  _amount,
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const assetParachainHex = `0000${numberToHex(
      Number(assetParachainId),
    ).substring(2)}`
    ...
    //amount
    assetStr.toLowerCase() === 'xckbtc'
      ? (mantissa = mantissa8)
      : (mantissa = mantissa12)

    if (assetStr.toLowerCase() === 'movr')
    ...
    const destParachainHex = `0000${numberToHex(
      Number(destParachainId),
    ).substring(2)}`

    const multilocation = [`0x00${destParachainHex}`, `0x01${nakedhexvalue}00`]
    const destination = [1, multilocation]
    console.log(
      `Transfer_multiasset amount: ${amount} nakedhexvalue: ${nakedhexvalue} Multilocation: `,
      multilocation,
      ` should be 0x0160c4d758184d11761943be32f71ae877974e0fa4cad523e1c3ba6c5ed340545c00 for qVA946xk9bGQ8A4m4EP3q1A1LJwvyi3QBzTRsAr68VvqeEo   destination: `,
      destination,
    )
    ...
    const tx4 = await Xtokens.transfer_multiasset(
      assetMultilocation,
      amount,
      destination,
      weight,
...
const transfer_xcKSMtoKSM = async (
  _amount,
  destinationRelayAccount,
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const relayTokenPrecompileAddress =
    ...
    const tx4 = await Xtokens.transfer(
      relayTokenPrecompileAddress,
      amount,
      destination,
      weight,
    )
...
const transfer_MultipleAssets_FromParachainToParachain = async (
  _token = 'KBTC',
  feesToken = 'KINT',
  originParachain = 2000,
  parachain = 1000,
  EVMaccount = '0x1270dbdE6Fa704f9363e47Dd05493D5dae329A4d',
  amount = '1',
) => {
  return new Promise(async (resolve, reject) => {
    if (!polkadotInjector || !polkadotInjectorAddress) {
      console.log(
        `transfer_MultipleAssets_FromParachainToParachain polkadotInjector and/or polkadotInjectorAddress are null. Cannot proceed!!!`,
      )
    ...
    const txMultipleAssetsParachainToParachain = await api.tx.xTokens
      ...
      .signAndSend(
        ...
        { signer: polkadotInjector.signer },
        async ({ status, events = [], dispatchError }) => {
          ...
          if (dispatchError) {
            if (dispatchError.isModule) {
              ...
              console.log(
                `txMultipleAssetsParachainToParachain dispatchError1 ${section}.${name}: ${docs.join(
                  ' ',
                )}`,
              )
              ...
              console.log(
                `txMultipleAssetsParachainToParachain dispatchError2: `,
              ...
              async ({ phase, event: { data, method, section } }) => {
                ...
                if (method === 'ExtrinsicSuccess') {
                  ...
                  signedBlock.block.extrinsics.forEach(
                    ({ hash, method: { method, section } }, index) => {
                  ...
                  resolve(['Extrinsic Failed'])
                else if (method === 'Withdrawn' && section === 'currencies')
                  originFees = `${ethers.utils.formatUnits(
                    ethers.BigNumber.from(`${data[2]}`),
                    12,
                  )}`
                else if (method === 'Deposit' && section === 'treasury')
                  treasuryFees = `${ethers.utils.formatUnits(
                    ethers.BigNumber.from(`${data[0]}`),
                    12,
                  )}`
                else if (
...
const transfer_Asset_FromParachainToParachain = async (
  _token = 'KUSD',
  originParachain = 2000,
  parachain = 1000,
  EVMaccount = '0x1270dbdE6Fa704f9363e47Dd05493D5dae329A4d',
  ...
  return new Promise(async (resolve, reject) => {
    if (!polkadotInjector || !polkadotInjectorAddress) {
      console.log(
        `transfer_Currency_FromParachainToParachain polkadotInjector and/or polkadotInjectorAddress are null. Cannot proceed!!!`,
      )
      resolve('Polkadot Extension Error Please Refresh Dapp')
      return
    }
    ...
    if (parachain === 1000 || parachain === 2023) {
      ...
      const accountId32tohexvalue = getAccountIdtoHex(EVMaccount)
      beneficiary = {
        AccountId32: { network: 'Any', id: accountId32tohexvalue },
      }
      general_key === '0x000b'
        ? (mantissa = mantissa8)
        : (mantissa = mantissa12)
        ...
        async ({ status, events = [], dispatchError }) => {
          ...
          if (dispatchError) {
            if (dispatchError.isModule) {
              ...
              errorInfo = `${section}.${name}`
              console.log(
                `txAssetParachainToParachain dispatchError1 ${section}.${name}: ${docs.join(
                  ' ',
                )}`,
              )
              //Exampleof Error message:  txAssetParachainToParachain dispatchError1 xTokens.NotCrossChainTransfer: Not cross-chain transfer.
              ...
              errorInfo = dispatchError.toString()
              console.log(
                `txAssetParachainToParachain dispatchError2: `,
                dispatchError.toString(),
                'errorInfo: ',
                errorInfo,
              )
...
const transfer_Currency_FromParachainToParachain = async (
  currency,
  originChain = 2000,
  parachain = 1000,
  EVMaccount = '0x1270dbdE6Fa704f9363e47Dd05493D5dae329A4d',
        ...
        async ({ status, events = [], dispatchError }) => {
          ...
          if (dispatchError) {
            if (dispatchError.isModule) {
              ...
              errorInfo = `${section}.${name}`
              console.log(
                `txCurrencyParachainToParachain dispatchError1 ${section}.${name}: ${docs.join(
                  ' ',
                )}`,
              )
            } else {
              ...
              errorInfo = dispatchError.toString()
              console.log(
                `txCurrencyParachainToParachain dispatchError2: `,
                dispatchError.toString(),
                'errorInfo: ',
                errorInfo,
              )
            ...
            events.forEach(
              async ({ phase, event: { data, method, section } }) => {
                ...
                if (method === 'ExtrinsicSuccess') {
                  ...
                  resolve(['Extrisnic Failed'])
                else if (method === 'Withdrawn' && section === 'currencies')
                  originFees = `${ethers.utils.formatUnits(
                    ethers.BigNumber.from(`${data[2]}`),
                    12,
                  )}`
                //txResult +=` Withdrawn ${JSON.parse(data[0]).token} from ${data[1]} KSM:${ethers.utils.formatUnits(ethers.BigNumber.from(`${data[2]}`),12)}`;
                else if (method === 'Deposit' && section === 'treasury')
                  treasuryFees = `${ethers.utils.formatUnits(
                    ethers.BigNumber.from(`${data[0]}`),
                    12,
...
const transferFromRelayToParachain = async (
  apiRelay,
  parachain = 1000,
  EVMaccount = '0x1270dbdE6Fa704f9363e47Dd05493D5dae329A4d',
  amount = '1',
  ...
  return new Promise(async (resolve, reject) => {
    if (!polkadotInjector || !polkadotInjectorAddress) {
      console.log(
        `transferFromRelayToParachain polkadotInjector and/or polkadotInjectorAddress are null. Cannot proceed!!!`,
      )
      resolve('Polkadot Extension Error Please Refresh Dapp')
      return
    }
      ...
      .signAndSend(
        ...
        async ({ status, events = [], dispatchError }) => {
          ...
          if (dispatchError) {
            if (dispatchError.isModule) {
              // for module errors, we have the section indexed, lookup
              const decoded = apiRelay.registry.findMetaError(
                dispatchError.asModule,
              )
              const { docs, name, section } = decoded
              errorInfo = `${section}.${name}`
              console.log(
                `txRelaytoParachain dispatchError1 ${section}.${name}: ${docs.join(
                  ' ',
                )}`,
              )
            } else {
              ...
              errorInfo = dispatchError.toString()
              console.log(
                `txRelaytoParachain dispatchError2: `,
                dispatchError.toString(),
                ' errorInfo: ',
                errorInfo,
              )
            ...
            events.forEach(
              async ({ phase, event: { data, method, section } }) => {
                ...
                if (method === 'ExtrinsicSuccess') {
                  ...
                  ExtrinsicResult = 'Succeeded'
                  const extrinsicBlockHash = status.asFinalized
                  const signedBlock = await apiRelay.rpc.chain.getBlock(
                    extrinsicBlockHash,
                  )
                  signedBlock.block.extrinsics.forEach(
                    ({ hash, method: { method, section } }, index) => {
                  ...
                  originFees = `${ethers.utils.formatUnits(
                    ethers.BigNumber.from(`${data[1]}`),
                    12,
                  )}`
                else if (method === 'Transfer')
                  tranferredAmount = `${ethers.utils.formatUnits(
                    ethers.BigNumber.from(`${data[2]}`),
                    12,
                  )}`
                else if (method === 'Deposit' && section === 'treasury')
...
const transferFromParachainToRelay = async (
  originChain,
  accntId32,
  amount = '1',
) => {
  return new Promise(async (resolve, reject) => {
    if (!polkadotInjector || !polkadotInjectorAddress) {
      console.log(
        `transferFromParachainToRelay polkadotInjector and/or polkadotInjectorAddress are null. Cannot proceed!!!`,
      )
      resolve('Polkadot Extension Error Please Refresh Dapp')
      return
    }
        ...
        async ({ status, events = [], dispatchError }) => {
          ...
          if (dispatchError) {
            if (dispatchError.isModule) {
              ...
              errorInfo = `${section}.${name}`
              console.log(
                `transferFromParachainToRelay dispatchError1 ${section}.${name}: ${docs.join(
                  ' ',
                )}`,
              )
            } else {
              ...
              errorInfo = dispatchError.toString()
              console.log(
                `transferFromParachainToRelay dispatchError2: `,
                dispatchError.toString(),
                'errorInfo: ',
                errorInfo,
              )
          ...
          if (status.isFinalized) {
            ...
            events.forEach(
              async ({ phase, event: { data, method, section } }) => {
                ...
                else if (method === 'Withdrawn' && section === 'tokens')
                  tranferredAmount = `${ethers.utils.formatUnits(
                    ethers.BigNumber.from(`${data[2]}`),
                    12,
                  )}`
...
const transfer_MOVR_FromParachainToParachain = async (
  originChain,
  parachainID,
  EVMaccount,
  amount = '1',
  ...
  return new Promise(async (resolve, reject) => {
    if (!polkadotInjector || !polkadotInjectorAddress) {
      console.log(
        `transfer_MOVR_FromParachainToParachain polkadotInjector and/or polkadotInjectorAddress are null. Cannot proceed!!!`,
      )
      resolve('Polkadot Extension Error Please Refresh Dapp')
      return
    }
        ...
        async ({ status, events = [], dispatchError }) => {
          ...
          if (dispatchError) {
            if (dispatchError.isModule) {
              ...
              errorInfo = `${section}.${name}`
              console.log(
                `txAssetMOVRParachainToParachain dispatchError1 ${section}.${name}: ${docs.join(
                  ' ',
                )}`,
              )
            } else {
              ...
              errorInfo = dispatchError.toString()
              console.log(
                `txAssetMOVRParachainToParachain dispatchError2: `,
                dispatchError.toString(),
                'errorInfo: ',
                errorInfo,
              )
...
const transferFromPhalaToParachain = async (
  token,
  originParachain,
  parachain,
  account,
  ...
  return new Promise(async (resolve, reject) => {
    if (!polkadotInjector || !polkadotInjectorAddress) {
      console.log(
        `transferFromPhalaToParachain polkadotInjector and/or polkadotInjectorAddress are null. Cannot proceed!!!`,
      )
      resolve('Polkadot Extension Error Please Refresh Dapp')
      return
    }
        ...
        async ({ status, events = [], dispatchError }) => {
          ...
          if (dispatchError) {
            if (dispatchError.isModule) {
              ...
              errorInfo = `${section}.${name}`
              console.log(
                `txAssetPhalaToParachain dispatchError1 ${section}.${name}: ${docs.join(
                  ' ',
                )}`,
              )
            } else {
              ...
              errorInfo = dispatchError.toString()
              console.log(
                `txAssetPhalaToParachain dispatchError2: `,
                dispatchError.toString(),
                'errorInfo: ',
                errorInfo,
              )
            ...
            console.log(
              `txAssetPhalaToParachain ***** NO DISAPTCHEERROR *****: `,
            )
        ...
        async ({ status, events = [], dispatchError }) => {
          ...
          if (dispatchError) {
            if (dispatchError.isModule) {
              ...
              errorInfo = `${section}.${name}`
              console.log(
                `transferFromPhalaToRelay dispatchError1 ${section}.${name}: ${docs.join(
                  ' ',
                )}`,
              )
            } else {
              ...
              errorInfo = dispatchError.toString()
              console.log(
                `transferFromPhalaToRelay dispatchError2: `,
                dispatchError.toString(),
                'errorInfo: ',
                errorInfo,
              )
              ...
              async ({ phase, event: { data, method, section } }) => {
                ...
                if (method === 'ExtrinsicSuccess') {
                  ...
                  treasuryFees = `${ethers.utils.formatUnits(
                    ethers.BigNumber.from(`${data[0]}`),
                    12,
                  )}` //txResult +=` Deposit to treasury ${data[0]} KSM:${ethers.utils.formatUnits(ethers.BigNumber.from(`${data[0]}`),12)}`;
...
const tranferFromRelayToRelay = async (
  apiRelay,
  recipient = '',
  _amount = '1',
) => {
  if (!polkadotInjector || !polkadotInjectorAddress) {
    console.log(
      `tranferFromRelayToRelay polkadotInjector and/or polkadotInjectorAddress are null. Cannot proceed!!!`,
    )
  }
...
const rococo_transfer_Asset_FromParachainToParachain = async (
  _token = 'BSX',
  originParachain = 2000,
  parachain = 1000,
  account,
...
//BSX FROM BASILISK TO KARURA or PHALA
const rococo_transfer_currency = async (
  currency,
  parachain,
  account,
  amount = '1',
  ...
  return new Promise(async (resolve, reject) => {
    if (!polkadotInjector || !polkadotInjectorAddress) {
      console.log(
        `rococo_transfer_currency polkadotInjector and/or polkadotInjectorAddress are null. Cannot proceed!!!`,
      )
      resolve('Polkadot Extension Error Please Refresh Dapp')
      return
    }
        ...
        async ({ status, events = [], dispatchError }) => {
          ...
          if (dispatchError) {
            if (dispatchError.isModule) {
              ...
              errorInfo = `${section}.${name}`
              console.log(
                `txCurrencyBasiliskToParachain dispatchError1 ${section}.${name}: ${docs.join(
                  ' ',
                )}`,
              )
            } else {
              ...
              errorInfo = dispatchError.toString()
              console.log(
                `txCurrencyBasiliskToParachain dispatchError2: `,
                dispatchError.toString(),
                'errorInfo: ',
                errorInfo,
              )
            ...
            console.log(
              `txCurrencyBasiliskToParachain ***** NO DISAPTCHEERROR *****: `,
            )
...
//BSX FROM BASILISK TO KARURA or PHALA
const rococo_transfer_currencyWithFee = async (
  currency,
  parachain,
  account,
  amount = '1',
  ...
  return new Promise(async (resolve, reject) => {
    if (!polkadotInjector || !polkadotInjectorAddress) {
      console.log(
        `rococo_transfer_currencyWithFee polkadotInjector and/or polkadotInjectorAddress are null. Cannot proceed!!!`,
      )
      resolve('Polkadot Extension Error Please Refresh Dapp')
      return
    }
        ...
        async ({ status, events = [], dispatchError }) => {
          ...
          if (dispatchError) {
            if (dispatchError.isModule) {
              ...
              errorInfo = `${section}.${name}`
              console.log(
                `txCurrencyWithFeeBasiliskToParachain dispatchError1 ${section}.${name}: ${docs.join(
                  ' ',
                )}`,
              )
            } else {
              ...
              errorInfo = dispatchError.toString()
              console.log(
                `txCurrencyWithFeeBasiliskToParachain dispatchError2: `,
                dispatchError.toString(),
                'errorInfo: ',
                errorInfo,
              )
            ...
            console.log(
              `txCurrencyWithFeeBasiliskToParachain ***** NO DISAPTCHEERROR *****: `,
            )
...
//BSX FROM BASILISK TO KARURA or PHALA
const rococo_transfer_multiCurrencies = async (
  currency,
  parachain,
  account,
  amount = '1',
  ...
  return new Promise(async (resolve, reject) => {
    if (!polkadotInjector || !polkadotInjectorAddress) {
      console.log(
        `rococo_transfer_multiCurrencies polkadotInjector and/or polkadotInjectorAddress are null. Cannot proceed!!!`,
      )
      resolve('Polkadot Extension Error Please Refresh Dapp')
      return
    }
        ...
        async ({ status, events = [], dispatchError }) => {
          ...
          if (dispatchError) {
            if (dispatchError.isModule) {
              ...
              errorInfo = `${section}.${name}`
              console.log(
                `txMultiCurrenciesBasiliskToParachain dispatchError1 ${section}.${name}: ${docs.join(
                  ' ',
                )}`,
              )
            } else {
              ...
              errorInfo = dispatchError.toString()
              console.log(
                `txMultiCurrenciesBasiliskToParachain dispatchError2: `,
                dispatchError.toString(),
                'errorInfo: ',
                errorInfo,
              )
            ...
            console.log(
              `txMultiCurrenciesBasiliskToParachain ***** NO DISAPTCHEERROR *****: `,
            )
...
//#region ***** Transfer Multiple Assets from Parachain to Parachain //*****
//BSX FROM BASILISK TO KARURA or PHALA
const rococo_transfer_MultiAssetS_FromParachainToParachain = async (
  _token = 'BSX',
  originParachain = 2000,
  parachain = 1000,
  account,
        ...
        async ({ status, events = [], dispatchError }) => {
          ...
          if (dispatchError) {
            if (dispatchError.isModule) {
              ...
              errorInfo = `${section}.${name}`
              console.log(
                `txMultiAssetSParachainToParachain dispatchError1 ${section}.${name}: ${docs.join(
                  ' ',
                )}`,
              )
              //Exampleof Error message:  txAssetParachainToParachain dispatchError1 xTokens.NotCrossChainTransfer: Not cross-chain transfer.
              ...
              errorInfo = dispatchError.toString()
              console.log(
                `txMultiAssetSParachainToParachain dispatchError2: `,
                dispatchError.toString(),
                'errorInfo: ',
                errorInfo,
              )
            ...
            console.log(
              `txMultiAssetSParachainToParachain ***** NO DISAPTCHEERROR *****: `,
            )
...
//BSX FROM BASILISK TO KARURA or PHALA
const rococo_transfer_MultiAssetWithFee_FromParachainToParachain = async (
  _token = 'BSX',
  originParachain = 2000,
  parachain = 1000,
  account,
        ...
        async ({ status, events = [], dispatchError }) => {
          ...
          if (dispatchError) {
            if (dispatchError.isModule) {
              ...
              errorInfo = `${section}.${name}`
              console.log(
                `txMultiAssetWithFeeParachainToParachain dispatchError1 ${section}.${name}: ${docs.join(
                  ' ',
                )}`,
              )
              //Exampleof Error message:  txAssetParachainToParachain dispatchError1 xTokens.NotCrossChainTransfer: Not cross-chain transfer.
              ...
              errorInfo = dispatchError.toString()
              console.log(
                `txMultiAssetWithFeeParachainToParachain dispatchError2: `,
                dispatchError.toString(),
                'errorInfo: ',
                errorInfo,
              )
            ...
            console.log(
              `txMultiAssetWithFeeParachainToParachain ***** NO DISAPTCHEERROR *****: `,
            )
...
const setup_SubstrateChain = async (wsURL = 'MoonbaseAlpha') => {
  ...
  else if (wsURL === 'Karura')
    WS_URL = 'wss://karura.api.onfinality.io/public-ws'
  else if (wsURL === 'Kintsugi')
    WS_URL = 'wss://kintsugi.api.onfinality.io/public-ws'
  else if (wsURL === 'Phala') WS_URL = 'wss://khala.api.onfinality.io/public-ws'
  else if (wsURL === 'Shiden')
    WS_URL = 'wss://shiden.api.onfinality.io/public-ws'
  //testnets
  else if (wsURL === 'MoonbaseRelayTestnet')
    WS_URL = 'wss://frag-moonbase-relay-rpc-ws.g.moonbase.moonbeam.network'
  else if (wsURL === 'MoonbaseAlpha')
    WS_URL = 'wss://wss.api.moonbase.moonbeam.network'
  else if (wsURL === 'KaruraAlphanet')
    WS_URL = 'wss://crosschain-dev.polkawallet.io:9908'
  else if (wsURL === 'BifrostAlphanet')
    WS_URL = 'wss://moonriver.bifrost-rpc.testnet.liebi.com/ws'
  else if (wsURL === 'KintsugiAlphanet')
    WS_URL = 'wss://api-dev-moonbeam.interlay.io/parachain'
  else if (wsURL === 'Rococo_Phala') WS_URL = 'wss://rhala-api.phala.network/ws'
  else if (wsURL === 'Rococo_Basilisk')
    WS_URL = 'wss://rpc-01.basilisk-rococo.hydradx.io'
  else if (wsURL === 'Rococo_Karura')
    WS_URL = 'wss://karura-rococo.aca-dev.network'
...
const getAvailableBalance = async (
  ...
  if (!token || !account) {
    console.log('No token or account has been provided for getAvailableBalance')
    return null
  }
  ...
  if (token.toLowerCase() === 'ksm') {
    ...
    //Kusama
    const { nonce, data: balanceKusama } = await KusamaApi.query.system.account(
      kusama_Address,
    ) // Retrieve the account balance & nonce via the system module
    let KusamaBalance = null
    if (balanceKusama)
      KusamaBalance = Number(
        ethers.utils.formatUnits(balanceKusama.free.toString(), 12),
      ).toFixed(4)
    ...
    const {
      ...
      reserved: reserved1,
      frozen: frozen1,
    } = await KaruraApi.query.tokens.accounts(karura_Address, {
      Token: token.toLowerCase(),
    })
    const KaruraBalance = Number(
      ethers.utils.formatUnits(free1.toString(), 12),
    ).toFixed(4)
    ...
    //Karura
    const { nonce, data: balance } = await KaruraApi.query.system.account(
      karura_Address,
    ) // Retrieve the account balance & nonce via the system module
    let KaruraBalance = null
    if (balance)
      KaruraBalance = Number(
        ethers.utils.formatUnits(balance.free.toString(), 12),
      ).toFixed(4)
    ...
    const KaruraBalance = Number(
      ethers.utils.formatUnits(free1.toString(), 18),
    ).toFixed(4)
    // console.log(`Karura For account: ${karura_Address} Token: ${token} => ${timestamp}: balance free: ${KaruraBalance} reserved: ${reserved1} frozen: ${frozen1}`);
    ...
    const {
      ...
      reserved: reserved1,
      frozen: frozen1,
    } = await KaruraApi.query.tokens.accounts(karura_Address, { Token: 'kusd' })
    const KaruraBalance = Number(
      ethers.utils.formatUnits(free1.toString(), 12),
    ).toFixed(4)
    // console.log(`***** Karura For account: ${karura_Address} Token: ${token} => ${timestamp}: balance free: ${KaruraBalance} reserved: ${reserved1} frozen: ${frozen1}`);
    ...
    console.log(
      `Karura For account: ${karura_Address} Token: ${token} => ${timestamp}: balance free: ${KaruraBalance} reserved: ${reserved1} frozen: ${frozen1}`,
    )
    ...
    const KaruraBalance = Number(
      ethers.utils.formatUnits(free1.toString(), 8),
    ).toFixed(4)
    // console.log(`Karura For account: ${karura_Address} Token: ${token} => ${timestamp}: balance free: ${KaruraBalance} reserved: ${reserved1} frozen: ${frozen1}`);
...
const getRococo_AvailableBalance = async (
  account,
  token = null,
  metamaskAccount,
  network = null,
) => {
  if (!token || !account || !metamaskAccount) {
    console.log(
      'No token or account or metamaskAccount has been provided for getAvailableBalance',
    )
    return null
  }
  ...
  if (token.toLowerCase() === 'bsx') {
    ...
    const {
      nonce,
      data: balance,
    } = await Rococo_BasiliskApi.query.system.account(basilisk_Address) // Retrieve the account balance & nonce via the system module
    let BasiliskBalance = null
    if (balance)
      BasiliskBalance = Number(
        ethers.utils.formatUnits(balance.free.toString(), 12),
      ).toFixed(4)
    ...
    const {
      ...
      reserved: reserved1,
      frozen: frozen1,
    } = await Rococo_KaruraApi.query.tokens.accounts(karura_Address, {
      ForeignAsset: 2,
    })
    const KaruraBalance = Number(
      ethers.utils.formatUnits(free1.toString(), 12),
    ...
    // Phala
    const Phala_balanceObj = await Rococo_RhalaApi.query.assets.account(
      5,
      khala_Address,
    )
    let PhalaBalance = null
    if (Phala_balanceObj.toJSON())
      PhalaBalance = Number(
        ethers.utils.formatUnits(Phala_balanceObj.toJSON().balance, 12),
      ).toFixed(4)

    const balances = {
      token,
      timestamp: new Date(timestamp).toISOString,
      Basilisk: BasiliskBalance,
      Karura: KaruraBalance,
      Phala: PhalaBalance,
    }
    ...
    const {
      nonce,
      data: balance,
    } = await Rococo_KaruraApi.query.system.account(karura_Address) // Retrieve the account balance & nonce via the system module
    let KaruraBalance = null
    if (balance)
      KaruraBalance = Number(
        ethers.utils.formatUnits(balance.free.toString(), 12),
    ...
    const {
      free: freePHA,
      reserved: reservedPHA,
      frozen: frozenPHA,
    } = await Rococo_BasiliskApi.query.tokens.accounts(basilisk_Address, 1)
    const BasiliskBalance = Number(
      ethers.utils.formatUnits(freePHA.toString(), 12),
    ).toFixed(4)
    ...
    // Phala
    const { nonce, data: balance } = await Rococo_RhalaApi.query.system.account(
      khala_Address,
    ) // Retrieve the account balance & nonce via the system module
    let PhalaBalance = null
    if (balance)
    ...
    console.log(
      `Now => ${timestamp}: Phala  For account:${khala_Address} ${token} balance Free: ${PhalaBalance}`,
    )
...
//#region ***** Transfer from Relay to Parachain //*****   0x1270dbdE6Fa704f9363e47Dd05493D5dae329A4d is Pablo Moon Account
const stakeKSMfromKusama = async (
  apiRelay,
  parachain = 1000,
  EVMaccount = '0x1270dbdE6Fa704f9363e47Dd05493D5dae329A4d',
  amount = '1',
  ...
  return new Promise(async (resolve, reject) => {
    if (!polkadotInjector || !polkadotInjectorAddress) {
      console.log(
        `stakeKSMfromKusama polkadotInjector and/or polkadotInjectorAddress are null. Cannot proceed!!!`,
      )
      resolve('Polkadot Extension Error Please Refresh Dapp')
      return
    }
    ...
    if (Number(amount) < 1.01) {
      console.log(
        `Inorder to stake KSM at Karura the minimum amount is 1 KSM plus XCM fees`,
      )
    ...
    const {
      ...
      reserved: reserved1,
      frozen: frozen1,
    } = await KaruraApi.query.tokens.accounts(EVMaccount, { Token: 'ksm' })
    ...
    //XCM transfer
    const txRelaytoParachain = await apiRelay.tx.xcmPallet.limitedReserveTransferAssets(
      { V1: { parents: 0, interior: { X1: { Parachain: parachain } } } },
      beneficiary,
      {
        V1: [
    ...
    const txRelaytoParachain_Signed = await txRelaytoParachain.signAsync(
      polkadotInjectorAddress,
      { signer: polkadotInjector.signer },
    )
    ...
    const txStakeKSM_Signed = await txStakeKSM.signAsync(
      polkadotInjectorAddress,
      { signer: polkadotInjector.signer },
    )

    txRelaytoParachain_Signed.send(
      async ({ status, events = [], dispatchError }) => {
        // console.log(`Current status: `,status,` Current status is ${status.type}`);
        let errorInfo
        if (dispatchError) {
            ...
            async ({ phase, event: { data, method, section } }) => {
              ...
              if (method === 'ExtrinsicSuccess') {
                ...
                const {
                  ...
                  reserved: reserved1,
                  frozen: frozen1,
                } = await KaruraApi.query.tokens.accounts(EVMaccount, {
                  Token: 'ksm',
                })
                ...
                const netAmount = `${ethers.utils.formatUnits(
                  ethers.BigNumber.from(`${changeKSMonKarura}`),
                  12,
                )}`
                const fees_PaidatDestinationChain = amountWEI.sub(
                  changeKSMonKarura,
                )
                const feesPaidatDestinationChain = `${ethers.utils.formatUnits(
                  ethers.BigNumber.from(`${fees_PaidatDestinationChain}`),
                  12,
                )}`
                ...
                txStakeKSM_Signed.send(
                  async ({ status, events = [], dispatchError }) => {
                    ...
                    if (dispatchError) {
                      if (dispatchError.isModule) {
                        // for module errors, we have the section indexed, lookup
                        const decoded = KaruraApi.registry.findMetaError(
                          dispatchError.asModule,
                        )
                        const { docs, name, section } = decoded
                        errorInfo = `${section}.${name}`
                        console.log(
                          `txStakeKSM_Signed dispatchError1 ${section}.${name}: ${docs.join(
                            ' ',
                          )}`,
                        )
                      } else {
                        ...
                        errorInfo = dispatchError.toString()
                        console.log(
                          `txStakeKSM_Signed dispatchError2: `,
                          dispatchError.toString(),
                          ' errorInfo: ',
                          errorInfo,
                        )
                      ...
                      console.log(
                        `txStakeKSM_Signed ***** NO DISAPTCHEERROR *****: `,
                      )
                      ...
                      events.forEach(
                        async ({ phase, event: { data, method, section } }) => {
                          ...
                          if (method === 'ExtrinsicSuccess') {
                            ...
                            const signedBlock = await KaruraApi.rpc.chain.getBlock(
                              KaruraextrinsicBlockHash,
                            )
                            signedBlock.block.extrinsics.forEach(
                              (
                                { hash, method: { method, section } },
                                index,
                              ) => {
                                if (index === Number(extrinsicIndexinBlock))
                                  KaruraExtrinsicHash = `${hash.toString()}`
                              },
                            )
                            ...
                            const {
                              ...
                              reserved: reserved1,
                              frozen: frozen1,
                            } = await KaruraApi.query.tokens.accounts(
                              EVMaccount,
                              { Token: 'lksm' },
                            )
                            ...
                            const txStakingMsg = [
                              ...
                              `Updated Free LKSM Balance: ${ethers.utils.formatUnits(
                                ethers.BigNumber.from(`${freeLKSM}`),
                                12,
                              )}`,
                            ]
...
const stakeKSMfromKintsugi = async (
  _token = 'KUSD',
  originParachain = 2000,
  parachain = 1000,
  EVMaccount = '0x1270dbdE6Fa704f9363e47Dd05493D5dae329A4d',
  ...
  console.log(
    '************************* ==== > WE ARE INSIDE stakeKSMfromKusama',
  )
  return new Promise(async (resolve, reject) => {
    if (!polkadotInjector || !polkadotInjectorAddress) {
      console.log(
        `stakeKSMfromKintsugi polkadotInjector and/or polkadotInjectorAddress are null. Cannot proceed!!!`,
      )
      resolve('Polkadot Extension Error Please Refresh Dapp')
      return
    }
    ...
    if (Number(amount) < 1.01) {
      console.log(
        `In order to stake KSM at Karura the minimum amount is 1 KSM plus XCM fees`,
      )
      resolve('To stake KSM for LKSM the minimum amount is 1.01 KSM')
      return
    }
    ...
    //XCM transfer
    const txAssetParachainToParachain = await api.tx.xTokens.transferMultiasset(
      {
        V1: {
          id: {
            Concrete: {
    ...
    const txParachaintoParachain_Signed = await txAssetParachainToParachain.signAsync(
      polkadotInjectorAddress,
      { signer: polkadotInjector.signer },
    )
    ...
    txParachaintoParachain_Signed.send(
      async ({ status, events = [], dispatchError }) => {
        let errorInfo
        if (dispatchError) {
          if (dispatchError.isModule) {
            ...
            errorInfo = `${section}.${name}`
            console.log(
              `txParachaintoParachain_Signed dispatchError1 ${section}.${name}: ${docs.join(
                ' ',
              )}`,
            )
          } else {
            errorInfo = dispatchError.toString()
            console.log(
              `txParachaintoParachain_Signed dispatchError2: `,
              dispatchError.toString(),
              ' errorInfo: ',
              errorInfo,
            )
          ...
          console.log(
            `txParachaintoParachain_Signed ***** NO DISAPTCHEERROR *****: `,
          )
...
const stakeKSMfromMoonriver = async (
  originMoonriverAddress,
  destParachainId,
  _amount,
  destinationAccount,
) => {
  console.log(
    `************************* ==== > WE ARE INSIDE stakeKSMfromMoonriver`,
  )
  return new Promise(async (resolve, reject) => {
    if (!polkadotInjector || !polkadotInjectorAddress) {
      console.log(
        `stakeKSMfromMoonriver polkadotInjector and/or polkadotInjectorAddress are null. Cannot proceed!!!`,
      )
      resolve('Polkadot Extension Error Please Refresh Dapp')
      return
    }
    ...
    const {
      free: initialKSM,
      reserved: intialreserved,
      frozen: initialfrozen,
    } = await KaruraApi.query.tokens.accounts(destinationAccount, {
      Token: 'ksm',
    })
    ...
    tx4.wait(3).then(async (reslveMsg) => {
      ...
      const txSendKSMMsg = [
        `Moonriver Transaction hash:${reslveMsg.transactionHash} .`,
        `Finalised at Block Hash: ${reslveMsg.blockHash} Block#: ${reslveMsg.blockNumber} gasUsed: ${reslveMsg.gasUsed}`,
        `Sent ${_amount} KSM to Karura account ${destinationAccount}`,
        `Received ${ethers.utils.formatUnits(
          ethers.BigNumber.from(`${netAmount}`),
          12,
        )} KSM`,
        `Fees paid at destination: ${ethers.utils.formatUnits(
          ethers.BigNumber.from(`${feesPaidatDestinationChain}`),
          12,
        )}`,
      ]
      // console.log(txSendKSMMsg);
      ...
      txStakeKSM_Signed.send(async ({ status, events = [], dispatchError }) => {
        console.log(
          `Current txStakeKSM_Signed status: `,
          status,
...
const getBalanceinKarura = async (accountAddress = '', token = '') => {
  ...
  if (KaruraApi && accountAddress !== '' && token !== '') {
    const {
      ...
      reserved: reserved1,
      frozen: frozen1,
    } = await KaruraApi.query.tokens.accounts(accountAddress, {
      Token: token.toLowerCase(),
    })
    const amount = `${ethers.utils.formatUnits(
      ethers.BigNumber.from(`${freeBalance}`),
      12,
    )}`
...
const unstakeKSMfromLKSM = async (_amount = '') => {
  ...
  return new Promise(async (resolve, reject) => {
    if (!polkadotInjector || !polkadotInjectorAddress) {
      console.log(
        `unstakeKSMfromLKSM polkadotInjector and/or polkadotInjectorAddress are null. Cannot proceed!!!`,
      )
      return
    }
    ...
    await KaruraApi.tx.dex
      .swapWithExactSupply(path, amount, 0)
      .signAndSend(
        polkadotInjectorAddress,
        { signer: polkadotInjector.signer },
        ({ status, events, dispatchError }) => {
          ...
          if (dispatchError) {
            if (dispatchError.isModule) {
              ...
              errorInfo = `${section}.${name}`
              console.log(
                `unstakeKSMfromLKSM dispatchError1 ${section}.${name}: ${docs.join(
                  ' ',
                )}`,
              )
            } else {
              ...
              errorInfo = dispatchError.toString()
              console.log(
                `unstakeKSMfromLKSM dispatchError2: `,
                dispatchError.toString(),
                ' errorInfo: ',
                errorInfo,
              )
            ...
            console.log(`unstakeKSMfromLKSM ***** NO DISAPTCHEERROR *****: `)

          if (status.isFinalized) {
            let KaruraExtrinsicResult,
              extrinsicIndexinBlock,
              KaruraExtrinsicHash
            // Loop through all events
            events.forEach(
              async ({ phase, event: { data, method, section } }) => {
                ...
                if (method === 'ExtrinsicSuccess') {
                  ...
                  // console.log(`====> FREE LKSM ON KARURA: ${freeLKSM}`);
                  const freeLKSM = await getBalanceinKarura(
                    polkadotInjectorAddress,
                    'LKSM',
                  )
